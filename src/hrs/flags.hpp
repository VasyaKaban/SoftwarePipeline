...
namespace hrs
{
	...
	struct flags
	{
		...
		constexpr static underlying_mask_type FULL_MASK = std::numeric_limits<underlying_mask_type>::max();
		...
		constexpr bool operator==(flags fl) const noexcept
		{
			return mask == fl.mask;
		}
		...
		{
			return mask == static_cast<underlying_mask_type>(value);
		}
